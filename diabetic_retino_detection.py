...
class TransferLearning:
    def __init__(self):
        ...
        parser.add_argument('--dim', type=int,
                            help='Image dimensions to process')
        parser.add_argument('--lr', type=float, help='learning rate',
                            default=1e-4)
        parser.add_argument('--batch_size', type=int, help='batch size')
        parser.add_argument('--epochs', type=int, help='no of epochs to train')
        parser.add_argument('--initial_layers_to_freeze', type=int,
                            help='the initial layers to freeze')
        parser.add_argument('--model', help='Standard Model to load',
                            default='InceptionV3')
        parser.add_argument('--folds', type=int,
                            help='num of cross validation folds', default=5)
    ...
    def train_model(self, train_dir, val_dir, n_fold=5, batch_size=16,
                    epochs=40, dim=224, lr=1e-5, model='ResNet50'):
        if model == 'Resnet50':
            model_final = self.resnet_pseudo(dim=224, freeze_layers=10,
                                             full_freeze='N')
        if model == 'VGG16':
            model_final = self.VGG16_pseudo(dim=224, freeze_layers=10,
                                            full_freeze='N')
        if model == 'InceptionV3':
            model_final = self.inception_pseudo(dim=224, freeze_layers=10,
                                                full_freeze='N')
        ...
        early = EarlyStopping(monitor='val_loss', patience=10, mode='min',
                              verbose=1)
        logger = CSVLogger(f'{self.outdir}/keras-epochs_ib.log', separator=',',
                           append=False)
        ...
        model_final.fit_generator(train_generator,
                                  ...
                                  callbacks=callbacks,
                                  class_weight={0: 0.012, 1: 0.12, 2: 0.058,
                                                3: 0.36, 4: 0.43})
    ...
    def inference(self, model_path, test_dir, class_dict, dim=224):
        ...
        df['actual_class_index'] = df['actual_class'].apply(
            lambda x: int(class_dict[x]))
        ...
        df['pred_class'] = df['pred_class_index'].apply(
            lambda x: (inv_class_dict[x]))
        return df
    ...
    def main(self):
        ...
        model_to_store_path, class_dict = self.train_model(self.train_dir,
                                                           self.val_dir,
                                                           n_fold=self.folds,
        ...
        val_results_df = self.inference(model_to_store_path, folder_path,
                                        class_dict, self.dim)
        ...
        actual_class_index = np.array(
            val_results_df['actual_class_index'].values)
        print(pred_class_index)
        print(actual_class_index)
        accuracy = np.mean(actual_class_index == pred_class_index)
        kappa = cohen_kappa_score(pred_class_index, actual_class_index,
                                  weights='quadratic')
        # print("-----------------------------------------------------")
